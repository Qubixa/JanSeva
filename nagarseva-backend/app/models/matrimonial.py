from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, Enum as SQLEnum, Text, Float, LargeBinary
...
class MatrimonialUser(Base):
    ...
    # Profile image stored as binary in DB
    profile_image_data = Column(LargeBinary, nullable=True)
    profile_image_mime = Column(String(50), nullable=True)   # e.g. "image/jpeg"
    # Keep legacy URL field for backward compat
    profile_image = Column(String(255), nullable=True)

    # Bio data document (PDF) stored as binary
    bio_data_document = Column(LargeBinary, nullable=True)
    bio_data_mime = Column(String(100), nullable=True)       # e.g. "application/pdf"
    bio_data_filename = Column(String(255), nullable=True)

    # Kundali document (optional) stored as binary
    kundali_document = Column(LargeBinary, nullable=True)
    kundali_mime = Column(String(100), nullable=True)
    kundali_filename = Column(String(255), nullable=True)
...
class MatrimonialAgency(Base):
    ...
    # Agency logo stored as binary in DB
    logo_data = Column(LargeBinary, nullable=True)
    logo_mime = Column(String(50), nullable=True)            # e.g. "image/png"
    # Keep legacy URL field for backward compat
