...
async def admin_login(request: AdminLoginRequest, db: Session = Depends(get_db)):
    """Login for admin portal — uses email, rejects non-admin roles"""
    user = db.query(User).filter(User.email == request.email).first()  # ← query by email
    ...
    if user.role not in [UserRole.SUPER_ADMIN, UserRole.WARD_ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    return TokenResponse(
        ...
        user=UserResponse(
            ...
            ward_name=ward.name if ward else None, address=user.address,
            profile_image=user.profile_image, is_active=user.is_active,
            is_verified=user.is_verified
        )
    )
